...
Usage:
    ...
    python -m src.cli sync-cache          同步本地K线缓存
...
def cmd_sync_cache():
    import yaml
    from src.screener.data_fetcher import get_all_stocks, sync_daily_klines

    args = sys.argv[2:]
    sync_all = "--all" in args or "--universe" in args
    codes = [a for a in args if not a.startswith("--")]

    if not sync_all and not codes:
        print("用法: python -m src.cli sync-cache [--all] [code1 code2 ...]")
        print("  例子: python -m src.cli sync-cache 600379 000062")
        print("  例子: python -m src.cli sync-cache --all")
        sys.exit(1)

    with open(_PROJECT_ROOT / "config.yaml", encoding="utf-8") as f:
        cfg = yaml.safe_load(f)

    days = int(cfg.get("backtest", {}).get("lookback_years", 1) * 250)
    if days <= 0:
        days = 250
    ...
    if sync_all:
        ...
        code_col = next((c for c in ["code", "浠ｇ爜"] if c in stock_df.columns), None)
        if not code_col:
            print("股票池缺少 code 列，无法同步。")
            sys.exit(1)

        codes = [str(c).zfill(6) for c in stock_df[code_col].dropna().astype(str).tolist()]
        print(f"准备同步全市场缓存，共 {len(codes)} 只股票，窗口 {days} 根K线。")
    else:
        codes = [str(c).zfill(6) for c in codes]
        print(f"准备同步 {len(codes)} 只股票，窗口 {days} 根K线。")

    ok = 0
    failed = 0
    for idx, code in enumerate(codes, 1):
        try:
            df = sync_daily_klines(code, days=days)
            if df.empty:
                failed += 1
                logger.warning("  %s 同步后仍无数据", code)
                continue
            ok += 1
            logger.info("  [%s/%s] %s -> %s 行", idx, len(codes), code, len(df))
        except Exception as exc:
            failed += 1
            logger.warning("  %s 同步失败: %s", code, exc)

    print(f"同步完成: 成功 {ok} 只, 失败 {failed} 只")
...
def main():
    if len(sys.argv) < 2:
        ...
        print("  python -m src.cli sync-cache --all      # 同步全市场日K缓存")
