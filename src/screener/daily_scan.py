...
from ..strategy.ranker import rank_score as _rank_signal_score
...
class ScanConfig:
    ...
    watchlist_min_ml_confidence: float = 0.45
    watchlist_min_sequence_confidence: float = 0.47
    ...
    watchlist_min_volatility_contraction_score: int = 5
...
def run_daily_scan(
    ...
    for idx, code in enumerate(codes):
        ...
        try:
            ...
            if params.high_win_mode and scan_config.allow_relaxed_fallback and not signals:
    ...
    if scan_config.allow_relaxed_fallback and len(watchlist_signals) < scan_config.watchlist_top_n:
        need = scan_config.watchlist_top_n - len(watchlist_signals)
        loose_fill = []
