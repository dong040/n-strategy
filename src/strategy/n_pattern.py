...
try:
    ...
    build_kline_tensor = None
    _load_sequence_model = None
    _predict_sequence_prob = None
    logger = logging.getLogger(__name__)
    logger.info("K-line sequence model disabled: %s", exc)

try:
    from .ml_filter import load_model as _load_ml_model, predict_signal as _predict_ml_signal
except Exception as exc:  # pragma: no cover - optional runtime dependency
    _load_ml_model = None
    _predict_ml_signal = None
    logger = logging.getLogger(__name__)
    logger.info("ML meta-filter disabled: %s", exc)
...
class NPatternParams:
    ...
    # High-win preset: still selective, but keeps enough trades to be meaningful.
    high_win_min_strength: int = 85
    high_win_min_close_position_score: int = 0
    high_win_min_volatility_contraction_score: int = 5
    high_win_min_sequence_confidence: float = 0.48
    high_win_min_factor_score: int = 0
    high_win_min_resistance_pct: float = 3.0
    high_win_min_rr: float = 1.5
    high_win_min_ml_confidence: float = 0.45
...
def _predict_ml_win_prob(factors: dict) -> float:
    ...
    try:
        if _load_ml_model is None or _predict_ml_signal is None:
            return 0.5
...
def _predict_sequence_win_prob(
    ...
    if build_kline_tensor is None or _load_sequence_model is None or _predict_sequence_prob is None:
        return 0.5
