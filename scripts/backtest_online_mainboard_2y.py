...
from src.strategy.ranker import rank_score as score_row
        ...
        rows.append(
            {
                ...
                "rr_ratio": getattr(t, "rr_ratio", 0.0),
                "entry_to_resistance_pct": getattr(t, "entry_to_resistance_pct", 0.0),
...
def _topn_by_day(trades_df: pd.DataFrame, top_n: int = 3) -> pd.DataFrame:
    ...
    ranked["composite"] = ranked.apply(lambda row: score_row(row.to_dict()), axis=1)
...
def _strict_subset(trades_df: pd.DataFrame, params: NPatternParams) -> pd.DataFrame:
    ...
    mask = (
        ...
        & (trades_df["sequence_confidence"] >= params.high_win_min_sequence_confidence)
        & (trades_df["strength"] >= params.high_win_min_strength)
        & (trades_df["factor_score"] >= params.high_win_min_factor_score)
